#File 1-calc-nearest-stop.R
#Code to calculate the nearest public transport stop (of each type) to each OA
...
#Load Data
  #Import population centriod data
    #Set WD
...
#For each OA, calculate nearest stop
  #Useful Examples
    ...
    #http://stackoverflow.com/questions/13458702/determining-minimum-values-in-a-vector-in-r
...
#Useful for running scripts on subset of data:
...
#Do nearest stop calculation    
  ...
  #loop through each entry in pop_centriods_data
    for (i in 1:nrow(pop_centriods_data)) {
      ...
      #rail
        ...
        #tidy variables 
          rm(router,fileloc,try_read,transport)
      #ferry
        #calc dist
      ...
      #move to next item
        i <- i + 1 
    } #end loop for each entry in pop_centriods_data
  #tidy variables
    rm(i,single_oa,tmp1,item)
  #cbind distance data togetehr
